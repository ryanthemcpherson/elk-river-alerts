...
def add_duplicate_prevention_columns():
    ...
    if not is_latest_exists:
        ...
        try:
            ...
            print("SQL: ALTER TABLE firearm_listings ADD COLUMN IF NOT EXISTS is_latest BOOLEAN DEFAULT TRUE;")
        except Exception as e:
    ...
    if listing_hash_exists and is_latest_exists:
        ...
        try:
            ...
            result = supabase.table('firearm_listings').update({"is_latest": True}).neq('id', 0).execute()
            print(f"Updated existing records to set is_latest=True")
            
            # Note: For listing_hash, we'd need to calculate it for each record
            # This is too complex for a basic migration script
            print("Note: The listing_hash field will be populated automatically on the next data refresh.")
        except Exception as e:
...
def add_market_listings_columns():
    ...
    try:
        # Query a single record
        result = supabase.table('firearm_listings').select('market_listings_json').limit(1).execute()
    ...
    try:
        # Query a single record
        result = supabase.table('firearm_listings').select('market_listings_count').limit(1).execute()
    ...
    if not market_listings_json_exists:
        ...
        try:
            print("Please add the 'market_listings_json' column manually through the Supabase console.")
            print("SQL: ALTER TABLE firearm_listings ADD COLUMN IF NOT EXISTS market_listings_json JSONB;")
        except Exception as e:
    ...
    if not market_listings_count_exists:
        ...
        try:
            print("Please add the 'market_listings_count' column manually through the Supabase console.")
            print("SQL: ALTER TABLE firearm_listings ADD COLUMN IF NOT EXISTS market_listings_count INTEGER DEFAULT 0;")
        except Exception as e:
...
def increase_varchar_limits():
    ...
    print("Once executed, the 'value too long for type character varying(30)' error should be resolved.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Database migration for Elk River Guns Inventory Tracker")
    parser.add_argument("--add-dup-prevention", action="store_true", help="Add duplicate prevention columns")
    parser.add_argument("--add-market-columns", action="store_true", help="Add market listings columns")
    parser.add_argument("--increase-varchar-limits", action="store_true", help="Increase varchar column size limits")
