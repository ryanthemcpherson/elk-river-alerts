...
def get_historical_data(supabase, days=30):
    ...
    result = (
        supabase.table("firearm_listings").select("*").gte("date_scraped", cutoff_date).execute()
    )
...
def get_historical_price_trends(supabase, manufacturer=None, model=None, days=90):
    ...
    price_trends = (
        df.groupby("date")
        .agg({"list_price": "mean", "estimated_value": "mean", "price_difference_percent": "mean"})
        .reset_index()
    )
        ...
        results.update(
            {
                ...
                "avg_savings_pct": below_market["price_difference_percent"].mean() * -1
                if len(below_market) > 0
                else 0,
                "avg_premium_pct": above_market["price_difference_percent"].mean()
...
def get_type_distribution(df):
    ...
    return {"counts": type_counts, "percentages": type_percentages}
...
def get_caliber_distribution(df):
    ...
    return {"counts": caliber_counts, "percentages": caliber_percentages}
