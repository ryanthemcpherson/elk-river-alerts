import argparse
from firearm_values import search_armslist, get_market_listings, estimate_value
import json
...
def main():
    ...
    if args.compare:
        ...
        algo_estimate = estimate_value(args.manufacturer, args.model, args.caliber, use_online_sources=False)
        
        print(f"Algorithm Estimate: ${algo_estimate['estimated_value']:.2f}")
        if algo_estimate['value_range']:
            print(f"Algorithm Range: ${algo_estimate['value_range'][0]:.2f} - ${algo_estimate['value_range'][1]:.2f}")
        ...
        blended_estimate = estimate_value(args.manufacturer, args.model, args.caliber, use_online_sources=True)
        
        print(f"Blended Estimate: ${blended_estimate['estimated_value']:.2f}")
        if blended_estimate['value_range']:
            print(f"Blended Range: ${blended_estimate['value_range'][0]:.2f} - ${blended_estimate['value_range'][1]:.2f}")
